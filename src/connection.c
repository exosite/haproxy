...
int make_proxy_line_v2(char *buf, int buf_len, struct server *srv, struct connection *remote)
{
	...
	struct chunk *cert_trash;
	struct chunk *fp_trash;
#endif
	...
	if (srv->pp_opts & SRV_PP_V2_SSL) {
		...
		if (ssl_sock_is_ssl(remote)) {
			...
			if (srv->pp_opts & SRV_PP_V2_SSL_CERT) {
				cert_trash = get_trash_chunk();
				if (ssl_sock_get_remote_cert(remote, cert_trash) > 0) {
					ssl_tlv_len += make_tlv(&buf[ret+ssl_tlv_len], (buf_len - ret - ssl_tlv_len), PP2_TYPE_SSL_CERT, cert_trash->len, cert_trash->str);
				}
			}
			if (srv->pp_opts & SRV_PP_V2_SSL_FP) {
				fp_trash = get_trash_chunk();
				if (ssl_sock_get_cert_sha1(remote, fp_trash) > 0) {
					ssl_tlv_len += make_tlv(&buf[ret+ssl_tlv_len], (buf_len - ret - ssl_tlv_len), PP2_TYPE_SSL_FP, fp_trash->len, fp_trash->str);
				}
			}
